...
namespace TrainTrain.Tests.Acceptance
{
    public class TrainTrainShould
    {
        ...
        [Test]
        public async Task Reserve_seats_when_in_coach_with_max_capacity_under_threshold_of_70_percents()
        {
            var fuzzer = new Fuzzer();
            var (trainId, bookingReference) = FuzzTrainIdAndBookingReference(fuzzer);
            const int seatsRequestedCount = 1;
            ...
            Check.That(reservation)
                .IsEqualTo($"{{\"train_id\": \"{trainId}\", \"booking_reference\": \"{bookingReference}\", \"seats\": [\"1B\"]}}");
        }
